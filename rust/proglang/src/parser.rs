...
// TODO: allow padding after : in variable definition type notation
...
pub fn parser() -> impl Parser<char, Vec<Statement>, Error=Simple<char>> {
    ...
    let statement: Recursive<_,Statement,_> = recursive (|statement: Recursive<_,Statement,_>| {
        let expr: Recursive<_,Expr,_> = recursive (|expr: Recursive<_, Expr,_>| {
            let op = |c| just(c).padded_by(pad.clone());
        ...
        let vardef = text::keyword("var").ignore_then(text::whitespace())
            ...
            .ignore_then(text::ident().padded_by(pad.clone()).separated_by(just(',')).at_least(1))
        ...
        let enumdef = text::keyword("enum").ignore_then(text::whitespace())
            ...
            .then_ignore(just('=').padded_by(pad.clone()))
            .then(
                text::ident().padded_by(pad.clone())
                .then(just('=').ignore_then(expr.clone()).or_not())
                .separated_by(just(','))
                .allow_trailing()
                .delimited_by(just('{'),just('}').padded_by(pad.clone()))
            )
        ...
        let r#break = text::keyword("break").ignore_then(text::whitespace::<_,Simple<char>>())
            .ignore_then(just(';')).map(|_| Statement::Break).padded_by(pad.clone());

        let r#continue = text::keyword("continue").ignore_then(text::whitespace::<_,Simple<char>>())
            .ignore_then(just(';')).map(|_| Statement::Continue).padded_by(pad.clone());
        ...
        let varassign = text::ident().padded_by(pad.clone())
            .then_ignore(just('='))
            .then(expr.clone())
            .then_ignore(just(';'))
        ...
        let arrayassign = text::ident()
            ...
            .or(codeblock)
            .or(expr.then_ignore(just(';')).map(|e| Statement::Expr(e)).padded_by(pad.clone()))
    });
